/**
 *
 * Created by ila on 5/3/2015.
 */
'use strict';

class IncompleteReadBuffer extends Error {
...
ReplyParser.prototype._parseResult = function (type) {
    ...
    if (type === 43 || type === 45) { // + or -
        ...
        if (this.options.returnBuffers) {
            return this._buffer.buffer.slice(start, end);
        } else {
        ...
        if (this.options.returnBuffers) {
            ...
            return this._arrayBuffer2string(this._buffer.buffer.slice(start, end));
        }
    } else if (type === 42) { // *
        offset = this._offset;
        packetHeader = new Packet(type, this.parseHeader());
...
ReplyParser.prototype.append = function (arrayBuffer) {
    if (!arrayBuffer) {
        return;
    }

    var newBuffer = new Uint8Array(arrayBuffer);
    ...
    var remaining = this._bytesRemaining(),
        ...
        tmpBuffer = new Uint8Array(newLength);

    tmpBuffer.set(this._buffer.buffer.slice(this._offset));
    tmpBuffer.set(newBuffer, remaining - 1);
...
ReplyParser.prototype._arrayBuffer2string = function (buf) {
